...
- 元仕様: [`docs/要件定義.md`](./docs/要件定義.md)（本リポジトリはフェーズ1=MVPの実装）

---

## 1. これは何をするアプリか

1. **AIで下書き生成**: ① 解説したいPDFをアップロード → ② 中心テーマ・フォーマット・雰囲気をテキストで指示（＋任意で手書きスケッチ写真）→ AIがPDFを読み取り構造化データ（`LessonDoc`）を生成。PDFは Claude / Gemini ともネイティブに読み込みます（最大25MB目安）。
2. **既存HTMLの取り込み**: トップの「📂 HTML を読み込む」から、本アプリで書き出したHTML（**無劣化で復元**）や、他ツールで作ったHTML（デザイン維持／簡易構造化／AI構造化の3方式）を取り込んで編集できます。
3. **ノード単位で編集**: 見出し・段落・単語1語（複数語のドラッグ選択も可）・樹形図の枝・表のセル・注釈などをクリックして、右のインスペクタで編集。色は「意味（働き）」を表す役割パレットで管理。
4. **HTMLで配布**: 同じデータから自己完結HTML（CSSインライン・レスポンシブ・印刷可）を書き出し、生徒に配布。書き出したHTMLには教材データが埋め込まれるため、**配布物がそのままバックアップ**になります。
...
| `APP_PASSCODE` | **推奨**: サーバー環境変数キーを使う場合のアクセスパスコード（下記参照） |

> ⚠️ **`APP_PASSCODE` について**: サーバー環境変数にAPIキーを置くと、URLを知る第三者があなたのキーの利用枠でAI APIを呼べてしまいます。`APP_PASSCODE` に任意の合言葉を設定すると、全AI系APIがパスコード必須になります（利用者は「⚙ AI設定」で同じ合言葉を1回入力）。BYOK（方式B）だけで使う場合は不要です。あわせて全APIにIP単位の簡易レートリミット（30回/分）が入っています。
...
lib/                        ロジック（UIから独立した「背骨」）
  ...
  validate.ts               AI出力(JSON)の安全なパース・正規化・id補完・色検証
  exportHtml.ts             LessonDoc → 自己完結HTML（CSSインライン・レスポンシブ・印刷可・データ埋め込み）
  importHtml.ts             既存HTMLの取り込み（埋め込みデータ復元／デザイン維持／簡易構造化）
  lessonStream.ts           AI生成のSSEパイプライン（JSON修復・リトライ込み）
  sanitize.ts               raw HTMLのサニタイズ（XSS対策）＋CSS色検証
  apiGuard.ts               APIの入口ガード（APP_PASSCODE＋レートリミット）
  sample.ts                 サンプル教材
...
app/                        Next.js App Router
  ...
  import/page.tsx           HTMLインポート画面（復元/デザイン維持/構造化/AI）
  ...
  api/
    generate/route.ts       AI下書き生成（JSON厳格＋修復・リトライ）
    import/route.ts         HTMLのAI構造化インポート
    edit/route.ts           範囲指定AI編集（選択ノードのみ送信→差し替え）
    models/route.ts         使用可能モデル一覧の動的取得
    config/route.ts         サーバーにキーがあるか等の真偽だけ返す

tests/                      vitest による回帰テスト（npm test）
```

データモデル（`LessonDoc`）の詳細は [`lib/types.ts`](./lib/types.ts) と docs/要件定義.md §4 を参照。
