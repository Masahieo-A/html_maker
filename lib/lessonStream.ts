...
/**
 * ストリーミング中の（まだ不完全な）JSONテキストから、直近に検出できた
 * heading ブロックの text らしき文字列を拾う（体感向上のためのベストエフォート、
 * 正確なJSONパースは行わない）。見つからなければ undefined。
 */
function findLastHeadingText(fullText: string): string | undefined {
  const re = /"type"\s*:\s*"heading"[\s\S]{0,200}?"text"\s*:\s*"((?:\\.|[^"\\])*)"/g;
  ...
  try {
    return JSON.parse(`"${last}"`).slice(0, 40);
  } catch {
    return last.replace(/\\"/g, '"').replace(/\\\\/g, "\\").slice(0, 40);
  }
}
      ...
      try {
        ...
        for await (const chunk of aiIter) {
          ...
          if (fullText.length - lastReported >= 200) {
            // 生成の逐次可視化（改善提案 D-3）: 正確な逐次パースは行わず、
            // 蓄積テキスト中の "type": の出現数をブロック数の目安として、
            // 直近で検出できた heading のテキストを添えて進捗表示に使う。
            const blockCount = (fullText.match(/"type"\s*:\s*"/g) ?? []).length;
            const lastHeading = findLastHeadingText(fullText);
            send({ progress: fullText.length, blockCount, lastHeading });
